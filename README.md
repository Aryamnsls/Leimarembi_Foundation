...
### Recent Updates
* **Premium Glassmorphism Redesign**: Completely overhauled the UI with a dynamic edge-to-edge panoramic background (featuring the 7 Sisters / Manipuri landscapes) and frosted glass (`backdrop-filter`) components.
* **Khuramjari Welcome Overlay**: Implemented a spectacular full-screen welcome modal featuring a traditional Manipuri greeting, glowing animations, and a seamless transition into the website. This triggers automatically for new sessions or when a user scans the official QR Code (`?qr=1`).
* **Smart Sticky Navbar**: Upgraded the navigation bar to automatically hide when scrolling down (maximizing screen space for the beautiful backgrounds) and instantly reappear when scrolling up.
* **Salai Taret Flag Logo**: Integrated the 7 colors of the Manipuri Salai Taret flag into the official logo to represent the entire community. The branding text was also updated to be bold and prominent.
* **Dynamic Theme Toggle**: Added a fully functioning Light/Dark mode toggle to the navigation bar. Colors dynamically adapt (`var(--btn-primary-text)`, etc.) to ensure perfect visibility against the complex panoramic backgrounds.
* **Unicode Browser Tab Styling**: Implemented a clever Unicode pseudo-alphabet injection to render the browser's tab title text in bold ("𝗟𝗲𝗶𝗺𝗮𝗿𝗲𝗺𝗯𝗲𝗲 𝗙𝗼𝘂𝗻𝗱𝗮𝘁𝗶𝗼𝗻").
